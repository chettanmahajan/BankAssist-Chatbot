...
3. FastAPI checks Redis for a cached `(session_id + query)` hit if Redis is available.
...
8. If Redis is available, the complete answer and sources are written to Redis (TTL 1 hour).

## Index Build

`backend/build_index.py` walks `data/`, loads every TXT/PDF, cleans and chunks at 600 chars with 120 overlap, embeds with MiniLM-L6-v2 (384-dim, CPU), and persists the FAISS index to `Faiss_db/`. Re-run any time `data/` changes.
...
- Redis can run as the system service `redis-server.service` when response caching is enabled.
