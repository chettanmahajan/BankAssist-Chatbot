...
- Python 3.10 - 3.12 (tested on 3.12, recommended for Ubuntu 24.04 EC2)
- Git
- A Groq API key
...
`VECTORSTORE_DIR` stores the FAISS files generated by `backend.build_index` (`index.faiss` and `index.pkl`).
...
Redis is optional and is used only for response caching. If Redis is not running, the backend continues working and treats every request as a cache miss.
...
See `deployment_guide.md` for step-by-step AWS EC2 setup (Nginx + systemd + Redis + swap). See `docs/architecture.md` for the request flow.
